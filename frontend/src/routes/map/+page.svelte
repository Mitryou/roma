<script lang="ts">
    ...
    import type {CustomFeature, Location, Place} from '../../types';
    ...
    const places: Place[] = [
        ...locations.map(location => ({
            id: LOCATION_ID_PREFIX + location.id,
            title: location.title,
            description: location.description,
            link: location.link,
            coordinates: location.coordinates
        })),
        ...zonesFeatures.map(feature => ({
            id: ZONE_ID_PREFIX + feature.id,
    ...
    function drawMarkers(): void {
        locations.forEach((location: Location) => {
            const id = LOCATION_ID_PREFIX + location.id;

            const customIcon = document.createElement('div');
            customIcon.id = `marker-${id}`
            customIcon.className = 'marker';

            customIcon.addEventListener('click', (evt) => {
                evt.stopPropagation();
                onPlaceSelect(`location-${location.id}`)
            });
    ...
    function drawPolygons(): void {
        zonesFeatures.forEach((feature: CustomFeature) => {
            const id = ZONE_ID_PREFIX + feature.id;
            ...
            map.on('click', layerId, () => onPlaceSelect(id));
        });
    }

    function onPlaceSelect(placeId: string): void {
        isFullDescriptionView = false;
        selectedPlace = places.find(place => place.id === placeId);

        if (selectedPlace) {
            highlightBtn(selectedPlace.id);

            document.querySelector('.marker.marker--active')?.classList.remove('marker--active');

            if (placeId.startsWith(LOCATION_ID_PREFIX)) {
                document.getElementById(`marker-${placeId}`).classList.add('marker--active');
                map.flyTo({
                    center: selectedPlace.coordinates,
                    zoom: 12,
                    essential: true
                });
                selectedPlace.shortDescription = cutDescription(selectedPlace.description);
            }

            if (placeId.startsWith(ZONE_ID_PREFIX)) {
                featureFitBounds(selectedPlace.id);
                selectedPlace.shortDescription = cutDescription(selectedPlace.description);
            }
        }
    }

    function highlightBtn(placeId: string): void {
        document.querySelector('.btn.btn--active')?.classList.remove('btn--active');
        const btnEl = document.getElementById(placeId);
        btnEl?.scrollIntoView({behavior: 'smooth', block: 'nearest', inline: 'start'});
        btnEl?.classList.add('btn--active');
    }

    function featureFitBounds(placeId: string): void {
        const polygonLayer = map.getSource(`${placeId}-layer`);
        map.fitBounds(turf.bbox(polygonLayer._data), {padding: 100});
    }
...
<section class="h-[calc(100vh_-_7rem)] relative flex flex-col">
    ...
    <div class="sidebar">
        <div class="sidebar-container">
            <div class="sidebar-main-content" class:sidebar-main-content--full={isFullDescriptionView}>
                {#if selectedPlace}
                    <button type="button"
                            class="close-btn"
                    ...
                    {:else}
                        <p class="text-base">{selectedPlace.description}</p>
                    {/if}
                {:else}
                    <p class="text-sm">
